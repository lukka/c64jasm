...
interface DocumentSymbols {
	labels: Array<{name: string, addr: number, kind: 'label', uri: string, line: number, range: any, hasDef?: boolean}>;
	macros: Array<{name: string, kind: 'macro', uri: string, line: number, loc: any, hasDef?: boolean}>;
	variables: Array<{name: string, kind: 'variable', value: any, uri: string, line: number, loc: any, hasDef?: boolean}>;
...
connection.onDidChangeConfiguration(change => {
	if (hasConfigurationCapability) {
		...
		globalSettings = <C64jasmSettings>(
			(change.settings['c64jasm-devtools'] || defaultSettings)
		);
	}
	...
	return new Promise<C64jasmSettings>(async (resolve) => {
		...
		if (!result) {
			const config = await connection.workspace.getConfiguration({
				scopeUri: resource,
				section: 'c64jasm-devtools'
			});
			result = config || defaultSettings;
			documentSettings.set(resource, Promise.resolve(result as C64jasmSettings));
		}
		resolve(result as C64jasmSettings);
	});
}
...
documents.onDidSave((event) => {
	...
	| { type: 'label', symbol: {name: string, addr: number, kind: 'label', uri: string, line: number, hasDef?: boolean} }
	| { type: 'macro', symbol: {name: string, kind: 'macro', uri: string, line: number, hasDef?: boolean} }
	| { type: 'variable', symbol: {name: string, kind: 'variable', value: any, uri: string, line: number, hasDef?: boolean} }
	| { type: 'property', parent: {name: string, kind: 'variable', value: any, uri: string, line: number, hasDef?: boolean}, value: any, name: string }
	| null;

function findSymbol(symbols: DocumentSymbols, word: string): SymbolSearchResult {
    // Strip leading namespace operator if present, as c64jasm outputs global names without it
    const searchWord = word.startsWith('::') ? word.substring(2) : word;

    // 1. Try to find the full word as a top-level symbol first (e.g. labels with dots)
    const label = symbols.labels.find(l => l.name === searchWord || l.name.endsWith(`::${searchWord}`));
    if (label) return { type: 'label', symbol: label };

    const macro = symbols.macros.find(m => m.name === searchWord);
    if (macro) return { type: 'macro', symbol: macro };

    const variable = symbols.variables.find(v => v.name === searchWord || v.name.endsWith(`::${searchWord}`));
    ...
    if (searchWord.includes('.')) {
        ...
        if (parentVariable && parentVariable.value) {
            ...
            if (found) {
                return { type: 'property', parent: parentVariable, value: current, name: searchWord };
            }
        }
    }
    return null;
...
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
	...
	const symbols: DocumentSymbols = {
		...
		macros: macros ? macros.map((m: any) => ({
			name: m.name,
			kind: 'macro' as const,
			uri: m.loc ? URI.file(path.resolve(m.loc.source)).toString() : textDocument.uri,
			hasDef: !!m.loc,
			line: m.loc ? Math.max(0, m.loc.start.line - 1) : 0,
			loc: m.loc
		})) : [],
...
connection.onHover((textDocumentPosition: TextDocumentPositionParams): Hover | null => {
	...
	let symbols = documentSymbols.get(textDocumentPosition.textDocument.uri);
	if (!symbols && documentSymbols.size > 0) {
		// Fallback: symbols are built globally from the entry point, so any cached version contains all symbols
		symbols = documentSymbols.values().next().value;
	}
	...
	if (symbols) {
		...
		if (result) {
			if (result.type === 'label') {
				...
				const defLink = label.hasDef ? `\n\n[Go to definition](${label.uri}#L${label.line + 1})` : '';
				return {
					contents: {
						kind: MarkupKind.Markdown,
						value: `**Label** \`${label.name}\`\n\nAddress: \`$${label.addr.toString(16).toUpperCase().padStart(4, '0')}\`${defLink}`
					}
				};
			} else if (result.type === 'macro') {
				const macro = result.symbol;
				connection.console.log(`[Hover] Found macro: ${macro.name}`);
				const defLink = macro.hasDef ? `\n\n[Go to definition](${macro.uri}#L${macro.line + 1})` : '';
				return {
					contents: {
						kind: MarkupKind.Markdown,
						value: `**Macro** \`${macro.name}\`${defLink}`
					}
				};
				...
				const defLink = variable.hasDef ? `\n\n[Go to definition](${variable.uri}#L${variable.line + 1})` : '';
				return {
					contents: {
						kind: MarkupKind.Markdown,
						value: `**Variable** \`${variable.name}\`\n\nValue: \`${valStr}\`${defLink}`
					}
				};
