...
**HOWEVER**, if a change to the source file is made, the change has effect **only** if the debug session is **stopped and restarted**. Not paused and continued, but completely stopped and restarted.
...
4. **Continuously Inspect State**: After *every single step*, call `c64jasm-getRuntimeC64Memory` with `{ cpu: true }`. This is critical: you must observe the Processor Status Register (`P`) and general registers (`A`, `X`, `Y`) to spot when flags (like Carry or Zero) are clobbered or when unexpected values are loaded by intervening operations.
5. **Clean Up**: Once the root cause is identified (e.g., an instruction overwriting a necessary flag), use `c64jasm-manageBreakpoint` (`action: "remove"`) to clean up the targeted breakpoint, and finally `continue` the debugger to restore normal execution.
