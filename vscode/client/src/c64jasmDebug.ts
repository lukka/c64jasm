...
export class C64jasmDebugSession extends LoggingDebugSession {
    ...
    public constructor() {
        ...
        this.runtime = new C64jasmRuntime();
    ...
    protected async launchRequest(response: DebugProtocol.LaunchResponse, args: LaunchRequestArguments) {
        try {
            await utils.wrapOp(`launchRequest`, response, async () => {
                ...
                await this.runtime.start(args.program, !!args.stopOnEntry, vicePath, args.source, args.extensionMode);
            }, this);
        }
        finally {
        }
                ...
                await Promise.all(sourceBps.map(
                    async l => {
                        let { verified, line, id } =
                            await this.runtime.setBreakpoint(path, this.convertClientLineToDebugger(l.line));
    ...
    private async requestSpriteData(): Promise<void> {
        ...
        try {
            ...
            // Request sprite memory ($0800-$0FFF for common sprite area)
            const spriteMemory = await this.runtime.retrieveMemory(0x0800, 0x0fff, 0);
            WebAppPanel.currentPanel.webview.postMessage({
                memory: spriteMemory,
                tag: "spriteMemory"
            });
