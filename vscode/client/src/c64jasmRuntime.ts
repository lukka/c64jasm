...
class MonitorConnection extends EventEmitter {
            ...
            if (!h)
                ...
                if (!eaterResult) {
                    ...
                    try {
                        ...
                        // Only send stop events after initialization is complete
                    ...
                    try {
                        ...
                        this.buffer = this.buffer.slice(size);
                        // Only send stop events after initialization is complete
                        if (resp.hit && !this.isInitializing)
    ...
    public async setMemory(address: number, value: number, bankId: number = 0): Promise<void> {
        ...
        await this.sendRequest<cmd.MemorySetResponse>(memReq);
    }
    ...
    public async setRegister(register: string, value: number): Promise<void> {
        ...
        await this.sendRequest<cmd.RegistersSetResponse>(setReq);
    }
    ...
    async loadProgram(prgName: string, startAddress: number, stopOnEntry: boolean): Promise<void> {
        if (stopOnEntry) {
            ...
            const err = set.tryGetResponse().header.code;
        ...
        const err = start.tryGetResponse().header.code;
        if (err !== 0) throw new Error(`err code ${err}`);
        
        // Initialization complete - now allow stop events to be sent
...
function startC64jasmServer(
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            const disasmPath = prgPath.replace(/\.prg$/, '.disasm');
            ...
            const pollServer = async () => {
                while (performance.now() - startTime < TIMEOUT_MS) {
                    ...
                    try {
                        await queryC64jasmDebugInfo(sourceFile, prgPath, false);
...
function queryC64jasmDebugInfo(
    ...
    const attemptConnection = (): Promise<C64jasmDebugInfo> => {
        return new Promise((resolve, reject) => {
            try {
                const port = 6502;
    ...
    return attemptConnection().catch(async (err) => {
        if (autoStart && emitRunInTerminal) {
            // Try to start the server and retry
            try {
                await startC64jasmServer(sourceFile, outputPath, emitRunInTerminal, isDevelopmentMode);
...
export class C64jasmRuntime extends EventEmitter {
     ...
     */
    public async start(program: string, stopOnEntry: boolean, vicePath: string, sourceFile: string, extensionMode?: vscode.ExtensionMode): Promise<void> {
    ...
    public async terminate(): Promise<void> {
        ...
        if (viceProcessId && viceProcessId > 0) {
            try {
                console.log(`Stopping x64sc (pid=${viceProcessId})`);
                process.kill(viceProcessId, 'SIGTERM');
            } catch (err) {
    ...
    public async stackTrace(): Promise<StackFrame[]> {
        ...
        if (res) {
            const { src, line } = res;
            const frame = new StackFrame(0, src.name, src, line);
            const instrAddr = this.findInstructionStartAddress(currentPC);
        ...
        for (const stackFrame of this.callStack) {
            ...
            if (res) {
                const { src, line } = res;
                const frame = new StackFrame(frameIndex, frameName, src, line);
    ...
    private findInstructionStartAddress(targetAddr: number): number {
        ...
        // Find the closest address by minimum absolute distance
        let bestAddr = addresses[0];
        let minDistance = Math.abs(targetAddr - bestAddr);
        
        for (const addr of addresses) {
            const distance = Math.abs(targetAddr - addr);
            ...
            if (bps) {
                for (const bp of bps) {
                    if (!bp.verified) {
                        ...
                        if (addr) {
                            bp.verified = true;
                            await this.monitor.setBreakpoint(addr);
    ...
    public async disassemble(memoryReference: string, instructionCount: number, offset: number): Promise<any[]> {
        ...
        const disasmFile = this._debugInfo?.outputPrg.replace(/\.prg$/, '.disasm');
    ...
    private getFunctionNameForAddress(address: number): string {
        ...
        // Search for the symbol that contains this address
        for (const symbol of this._debugInfo.symbols) {
            const symbolSize = symbol.size || 1;
            if (symbol.addr <= address && address < symbol.addr + symbolSize) {
                return symbol.name;
            }
        }
    ...
    private async readStackReturnAddresses(): Promise<number[]> {
        ...
        try {
            ...
            const stackStart = spValue + 1;
            const stackEnd = Math.min(0x1FF, stackStart + 40); // Limit to 20 frames
            ...
            if (stackStart > 0x1FF) {
                console.log(`DEBUG: readStackReturnAddresses() - stackStart > 0x1FF, stack empty`);
                return returnAddresses;
            }
            ...
            for (let i = 0; i <= stackMemory.length - 4; i += 2) {
                ...
                if (!isNaN(lo) && !isNaN(hi)) {
                    ...
                    if (returnAddr >= 0x0800 && returnAddr <= 0xCFFF) {
                        ...
                        // Treat size=0 symbols as having at least 1 byte
                        let foundInSymbol = false;
                        for (const symbol of this._debugInfo.symbols) {
                            const symbolSize = symbol.size || 1;
                            if (symbol.addr <= returnAddr && returnAddr < symbol.addr + symbolSize) {
                                foundInSymbol = true;
                                console.log(`DEBUG: readStackReturnAddresses() - 0x${returnAddr.toString(16).toUpperCase()} found in symbol: ${symbol.name} (addr:0x${symbol.addr.toString(16).toUpperCase()}, size:${symbol.size})`);
                                break;
                            }
                        }
