...
class MonitorConnection extends EventEmitter {
    ...
    // Running state captured just before the first binary-protocol operation in a batch.
    // Used by restoreRunningState() to decide whether to resume after the batch completes.
    private wasRunningBeforeOperation: boolean = false;
    // Number of binary-protocol operations currently in flight.
    private operationInProgress: number = 0;
    // Set to true when the user explicitly presses the Stop/Pause button.
    private userRequestedStop: boolean = false;
    // Set to true when a real stop has occurred (breakpoint hit or user pause) while
    // an operation was in flight, so restoreRunningState() knows NOT to resume.
    private breakpointHit: boolean = false;
    ...
    public async restoreRunningState(): Promise<void> {
        this.operationInProgress--;
        if (this.operationInProgress === 0 && this.wasRunningBeforeOperation && this.stopped && !this.breakpointHit) {
            await this.go();
        }
    }

    private shouldResumeAfterSpuriousPause(): boolean {
        return this.wasRunningBeforeOperation && this.operationInProgress > 0 && !this.breakpointHit;
    }
                    ...
                    try {
                        ...
                        if (this.isInitializing && this.expectingStopOnEntry && this.hasResumedAfterCheckpoint) {
                            ...
                            if (this.userRequestedStop) {
                                ...
                                // AND no real stop event is pending — resume immediately.
                                this.go();
                            }
    ...
    private async withSavedRunningState<T>(action: () => Promise<T>): Promise<T> {
    ...
    public async getAvailableBanks(): Promise<cmd.BankInfo[]> {
        return this.withSavedRunningState(async () => {
        ...
        return this.withSavedRunningState(async () => {
            ...
            function dataEater(buffer: Buffer): number {
                try {
                    ...
                    console.log(`Parsed header: ${cmd.toString(header)}`);
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            const args = isDevelopmentMode
                ...
                : (useEmbeddedCompiler !== false
                    ? [
                        'node',
                        // In production, run the bundled cli.js file packaged inside the extension.
                        path.join(__dirname, '..', '..', 'node_modules', 'c64jasm', 'dist', 'src', 'cli.js'),
                        '--watch', sourceDir,
                        '--out', prgPath,
                        '--server', sourceFile,
                        '--disasm', finalDisasmPath
...
function queryC64jasmDebugInfo(
    ...
    return performConnection().catch(async (err) => {
        if (autoStart && emitRunInTerminal) {
            ...
            try {
                await startC64jasmServer(sourceFile, outputPath, disasmPath, emitRunInTerminal, isDevelopmentMode, useEmbeddedCompiler);
...
export class C64jasmRuntime extends EventEmitter {
    ...
    public async start(program: string, stopOnEntry: boolean, vicePath: string, sourceFile: string, disasmPath: string | undefined, extensionMode?: vscode.ExtensionMode, useEmbeddedCompiler?: boolean): Promise<void> {
        ...
        if (this._debugInfo && this._debugInfo.error) {
            throw new Error(`Compilation failed: ${this._debugInfo.error}. Please check the c64jasm debug srv terminal for details.`);
        }
    ...
    public async terminate(): Promise<void> {
        ...
        if (vicePidToKill && vicePidToKill > 0) {
            ...
            if (!processDied) {
                try {
                    console.log(`x64sc (pid=${vicePidToKill}) did not quit gracefully, sending SIGTERM`);
