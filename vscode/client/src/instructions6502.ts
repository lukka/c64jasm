...
import { c64Hardware, HardwareRegister } from './c64hardware';

/**
 * Helper function to format instruction info for hover display
 */
export function formatInstructionHover(info: InstructionInfo): string {
    let text = `${info.name} — ${info.fullName}\n`;
    ...
    if (info.addressingModes && info.addressingModes.length > 0) {
        ...
        info.addressingModes.forEach(mode => {
            text += `  ${mode.mode.padEnd(15)} ${mode.opcode}  (${mode.bytes} bytes, ${mode.cycles} cycles)\n`;
        });
        text += `\n`;
    }
    ...
    if (info.flags && info.flags.length > 0) {
        text += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
        text += `PROCESSOR FLAGS\n\n`;
        info.flags.forEach(flag => {
            text += `  ${flag.flag} - ${flag.name}\n      ${flag.description}\n`;
        });
        text += `\n`;
    }
    ...
    if (info.seeAlso && info.seeAlso.length > 0) {
        text += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
        text += `See also: ${info.seeAlso.join(', ')}\n`;
    }
...
/**
 * Helper function to format hardware register info for hover display
 */
export function formatHardwareRegisterHover(addr: string, register: HardwareRegister): string {
    ...
    let text = `${chipEmoji[register.chip]} ${addr.toUpperCase()} — ${register.name}\n`;
    text += `Chip: ${register.chip} | Access: ${register.access}\n\n`;
    text += `${register.description}\n\n`;

    // Bit configuration section
    if (register.bits && register.bits.length > 0) {
        text += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
        text += `BIT CONFIGURATION\n\n`;
        register.bits.forEach(bit => {
            text += `  ${bit.range}: ${bit.description}\n`;
        });
        text += `\n`;
    }
...
/**
 * Lookup a hardware register by address
 */
export function lookupHardwareRegister(addr: string): HardwareRegister | undefined {
    ...
    return c64Hardware[normalizedAddr];
}
