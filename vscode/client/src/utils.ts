...
}export class MutableDisposable<T extends { dispose(): any }> {
    private _value?: T;
...
export class AsyncMutableDisposable<T extends { dispose(): Promise<any> }> {
    private _value?: T;
    ...
    async setValue(newValue: T | undefined): Promise<void> {
        if (this._value !== newValue) {
            const oldValue = this._value;
            this._value = newValue;
            if (oldValue) {
                await oldValue.dispose();
            }
        }
    }
...
export function evaluateMathExpression(
    expr: string, 
    resolveSymbol: (name: string) => { addr?: number, value?: any } | null
): number | undefined {
    try {
        const processedExpr = expr
            .replace(/\$([0-9a-fA-F]+)/g, (_, hex) => parseInt(hex, 16).toString())
            .replace(/0x([0-9a-fA-F]+)/gi, (_, hex) => parseInt(hex, 16).toString())
            .replace(/%([01]+)\b/g, (_, bin) => parseInt(bin, 2).toString())
            .replace(/[a-zA-Z_][a-zA-Z0-9_:.]*/g, (match) => {
                const sym = resolveSymbol(match);
                if (sym) {
                    return sym.addr !== undefined ? sym.addr.toString() : 
                           (typeof sym.value === 'number' ? sym.value.toString() : 'NaN');
                }
                return match; 
            });
