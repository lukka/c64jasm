...
export enum RequestType {
    ...
    MON_REQUEST_CPUHISTORY_GET = 0x86,
...
export enum ResponseType {
    ...
    MON_RESPONSE_CPUHISTORY_GET = 0x86,
...
class BufferReader {
    ...
    public readBigUInt64LE(): bigint {
        const n = this.buffer.readBigUInt64LE(this.readPtr);
        this.readPtr += 8;
        return n;
    }
...
export class BanksAvailableRequest extends Request<BanksAvailableResponse> {
    constructor() {
        const bw = new BufferWriter(Buffer.alloc(0));
        super(RequestType.MON_REQUEST_BANKS_AVAILABLE, bw.buffer);
    }

    public createResponse(buffer: Buffer): BanksAvailableResponse {
        return new BanksAvailableResponse(buffer);
    }
}

export class BanksAvailableResponse extends Response {
    public readonly banks: BankInfo[] = [];

    constructor(buffer: Buffer) {
        super(buffer);
        let br = new BufferReader(buffer.slice(HeaderRespLen));
        const bc = br.readUInt16LE();
        
        for (let i = 0; i < bc; i++) {
            const itemSize = br.readUInt8();
            const bankId = br.readUInt16LE();
            const nameLen = br.readUInt8();
            const name = br.readString(nameLen);
            this.banks.push({ id: bankId, name });
            
            // The item size tells us the total length of the payload minus the size byte itself.
            // We've read 2 bytes for the id, 1 byte for nameLen, and nameLen bytes for the name.
            // So we need to skip any padding/extra bytes to reach the next item.
            const bytesRead = 2 + 1 + nameLen;
            if (itemSize > bytesRead) {
                br.readBuffer(itemSize - bytesRead);
            }
        }
    }
...
export class MemoryGetRequest extends Request<MemoryGetResponse> {
    constructor(id: number, start: number, end: number, bankId: number) {
        ...
        this.description = `Get Memory: [0x${start.toString(16)} - 0x${end.toString(16)}] bank ${bankId}`;
    }
...
export class MemorySetRequest extends Request<MemorySetResponse> {
    constructor(start: number, end: number, bankId: number, memory: Buffer) {
        ...
        this.description = `Set Memory: [0x${start.toString(16)} - 0x${end.toString(16)}] bank ${bankId}`;
    }
...
export class StepOutRequest extends Request<StepOutResponse> {
    constructor() {
        ...
        this.description = "Step Out (run until return)";
    }
...
export class ContinueRequest extends Request<ContinueResponse> {
    constructor() {
        ...
        this.description = "Continue Execution";
    }
...
export class RegistersGetRequest extends Request<RegistersGetResponse> {
    constructor() {
        ...
        this.description = "Get CPU Registers";
    }
...
export interface CpuHistoryItem {
    regs: { [id: number]: { value: number } };
    clock: bigint;
    instructionBytes: Buffer;
}

export class CpuHistoryGetRequest extends Request<CpuHistoryGetResponse> {
    constructor(historyCount: number) {
        // MS | HC HC HC HC
        const b = Buffer.alloc(5);
        b.writeUInt8(0, 0); // MS: 0 = main memory
        b.writeUInt32LE(historyCount, 1);
        super(RequestType.MON_REQUEST_CPUHISTORY_GET, b);
        this.description = `Get CPU History (count: ${historyCount})`;
    }

    public createResponse(buffer: Buffer): CpuHistoryGetResponse {
        return new CpuHistoryGetResponse(buffer);
    }
}

export class CpuHistoryGetResponse extends Response {
    public readonly items: CpuHistoryItem[] = [];

    constructor(buffer: Buffer) {
        super(buffer);

        const body = buffer.slice(HeaderRespLen);
        const br = new BufferReader(body);

        const hc = br.readUInt32LE();

        for (let i = 0; i < hc; i++) {
            const is = br.readUInt8(); // Item size, excluding this byte
            const itemBuffer = br.readBuffer(is);
            const itemBr = new BufferReader(itemBuffer);

            const rc = itemBr.readUInt16LE();
            const regs: { [id: number]: { value: number } } = {};
            for (let j = 0; j < rc; j++) {
                const rs = itemBr.readUInt8();
                const regBuffer = itemBr.readBuffer(rs);
                const regBr = new BufferReader(regBuffer);

                const id = regBr.readUInt8();
                const value = regBr.readUInt16LE();
                // note: the Register item body size is 3 according to doc, but if RS is e.g. 3, we read id (1 byte) and value (2 bytes).
                regs[id] = { value };
            }

            const clock = itemBr.readBigUInt64LE();
            const il = itemBr.readUInt8();
            const instructionBytes = itemBr.readBuffer(il);

            this.items.push({ regs, clock, instructionBytes });
        }
    }

    public override Type(): ResponseType { return ResponseType.MON_RESPONSE_CPUHISTORY_GET; }
}
...
export class RegistersAvailRequest extends Request<RegistersAvailResponse> {
    constructor() {
        ...
        this.description = "Get Available Registers";
    }
...
export class RegistersSetRequest extends Request<RegistersSetResponse> {
    constructor(registerId: number, value: number) {
        ...
        this.description = `Set Register: id=${registerId} to 0x${value.toString(16)}`;
    }
...
export class VICEInfoGetRequest extends Request<VICEInfoGetResponse> {
    constructor() {
        ...
        this.description = "Get VICE Info";
    }
...
export class BankGetRequest extends Request<BankGetResponse> {
    constructor() {
        ...
        this.description = "Get Memory Banks";
    }
...
export class BankGetResponse extends Response {
    ...
    public constructor(buffer: Buffer) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            const bytesRead = 2 + 1 + len;
            if (sz > bytesRead) {
                br.readBuffer(sz - bytesRead);
            }
        }
    }
...
export class QuitRequest extends Request<QuitResponse> {
    constructor() {
        ...
        this.description = "Quit VICE monitor";
    }
...
export class AutoStartRequest extends Request<AutoStartResponse> {
    constructor(file: string, run: boolean) {
        ...
        this.description = `AutoStart file: ${file} (run on load: ${run})`;
    }
...
export class CheckpointListRequest extends Request<CheckpointListFakeResponse> {
    ...
    constructor() {
        ...
        this.description = "Get Checkpoint List";
    }
...
export class CheckpointSetRequest extends Request<CheckpointResponse> {
    constructor(start: number, end: number, stop: boolean, enabled: boolean, op: CPUOp, temp: boolean) {
        ...
        this.description = `Set Checkpoint: [0x${start.toString(16)} - 0x${end.toString(16)}] op=${op}`;
    }
...
export class CheckpointResponse extends Response {
    ...
    public override toString(): string {
        ...
        return `${base}\n\t Checkpoint(id=${this.checkpointId}, start=0x${this.start.toString(16)}, end=0x${this.end.toString(16)}, enabled=${this.enabled}, stopWhenHit=${this.stopWhenHit}, CPUOp=${this.cpuOp})`;
    }
};
...
export class CheckpointDeleteRequest extends Request<CheckpointDeleteResponse> {
    constructor(checkpointId: number) {
        ...
        this.description = `Delete Checkpoint: id=${checkpointId}`;
    }
...
export class CheckpointToggleRequest extends Request<CheckpointDeleteResponse> {
    constructor(checkpointId: number, enabled: boolean) {
        ...
        this.description = `Toggle Checkpoint: id=${checkpointId} enabled=${enabled}`;
    }
...
export class ConditionSetRequest extends Request<ConditionSetResponse> {
    constructor(checkpointId: number, condition: string) {
        ...
        conditionBuffer.copy(bw.buffer as Uint8Array, 5);
        super(RequestType.MON_REQUEST_CONDITION_SET, bw.buffer);
        this.description = `Set Condition: id=${checkpointId} condition="${condition}"`;
    }
...
export class DisplayGetRequest extends Request<DisplayGetResponse> {
    constructor(useVicII: boolean, format: number) {
        let body = Buffer.alloc(2);
        body.writeUInt8(useVicII ? 1 : 0, 0);
        body.writeUInt8(format, 1);
        super(RequestType.MON_REQUEST_DISPLAY_GET, body);
        this.description = `Get Display: vicII=${useVicII} format=${format}`;
    }
    public createResponse(buffer: Buffer): DisplayGetResponse {
        return new DisplayGetResponse(buffer);
    }
}

export class DisplayGetResponse extends Response {
    public debugWidth: number = 0;
    public debugHeight: number = 0;
    ...
    constructor(buffer: Buffer) {
        ...
        if (this.body && this.body.length > 0) {
            let br = new BufferReader(this.body);
            // Assuming API version >= 0x02
            let metaLength = br.readUInt32LE();
            this.debugWidth = br.readUInt16LE();
            this.debugHeight = br.readUInt16LE();
            this.offsetX = br.readUInt16LE();
            this.offsetY = br.readUInt16LE();
            this.innerWidth = br.readUInt16LE();
            this.innerHeight = br.readUInt16LE();
            this.bpp = br.readUInt8();
            // remainingMeta should be metaLength (which is 13 for API v2) minus the 13 bytes we just read.
            let remainingMeta = metaLength - 13;
            if (remainingMeta > 0) {
                br.readBuffer(remainingMeta);
            }
            let imgLen = br.readUInt32LE();
            this.imageData = br.readBuffer(imgLen);
        }
    }
    public override Type(): ResponseType { return ResponseType.MON_RESPONSE_DISPLAY_GET; }
}
