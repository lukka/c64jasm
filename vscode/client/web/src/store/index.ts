...
export default createStore({
  state: {
    ...
    SpriteMemory: new Uint8Array(2048), // Sprite data memory
  ...
  mutations: {
    ...
    setSpriteMemory(state, memory) {
      state.SpriteMemory = memory;
    },
  ...
  plugins: [
    (store) => {
      ...
      window.addEventListener("message", (event) => {
        ...
        if (data.tag === "spriteMemory" && data.memory) {
          const bytes = parseMemoryHex(data.memory);
          store.commit("setSpriteMemory", new Uint8Array(bytes));
        }
