        ...
        <div class="memory-grid">
          <div class="memory-scroll-container">
            <div class="memory-header">
              ...
              <span class="hex-col">
                <span class="hex-header-byte">00</span>
                <span class="hex-header-byte">01</span>
                <span class="hex-header-byte">02</span>
                <span class="hex-header-byte">03</span>
                <span class="hex-header-byte">04</span>
                <span class="hex-header-byte">05</span>
                <span class="hex-header-byte">06</span>
                <span class="hex-header-byte">07</span>
                <span class="hex-header-byte">08</span>
                <span class="hex-header-byte">09</span>
                <span class="hex-header-byte">0A</span>
                <span class="hex-header-byte">0B</span>
                <span class="hex-header-byte">0C</span>
                <span class="hex-header-byte">0D</span>
                <span class="hex-header-byte">0E</span>
                <span class="hex-header-byte">0F</span>
              </span>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const spriteMemoryFromStore = computed(
      () => store.state.SpriteMemory || new Uint8Array(2048)
    ); // Raw sprite data
    ...
    const spriteData = computed(() => {
      return Array(8)
        .fill(null)
        .map((_, idx) => {
          const ptr = spritePointersFromStore.value[idx];
          const absoluteAddress = ptr * 64;
          // Subtract base address of requested memory ($0800) to get offset in buffer
          const offset = absoluteAddress - 0x0800;
          const data: number[] = [];
          for (let i = 0; i < 63; i++) {
            data.push(spriteMemoryFromStore.value[offset + i] || 0);
          }
          return data;
        });
    });
    ...
    const requestSpriteData = () => {
      if (vscode) {
        ...
        // Request sprite memory (sprites typically in bank 0, $0000-$3FFF range)
        // We'll request the common sprite area $0800-$0FFF (8 sprites * 64 bytes = 512 bytes)
        vscode.postMessage({
          command: "getMemory",
          start: 0x0800,
          end: 0x0fff,
